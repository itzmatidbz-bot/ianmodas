document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadProduct() {
    ...
    try {
      ...
      if (!imagenesError && imagenes && imagenes.length > 0) {
        ...
        console.log(
          "🖼️ No se encontraron imágenes adicionales, usando imagen principal",
        );
      }
      ...
      loadRelatedProducts(
        currentProduct.categoria_nombre || currentProduct.categoria,
        currentProduct.id,
      );
    } catch (error) {
      console.error("Error al cargar el producto:", error);
      productDetailContainer.innerHTML =
        '<p class="error-message">No se pudo cargar el producto. Intente más tarde.</p>';
    }
  }
  ...
  function renderProductDetails() {
    ...
    if (currentProduct.imagenes && Array.isArray(currentProduct.imagenes)) {
      ...
      images = [
        {
          url: "https://placehold.co/600x800/eee/ccc?text=IanModas",
          es_principal: true,
        },
      ];
    }
    ...
    let coloresHTML = "";
    if (
      currentProduct.colores_disponibles &&
      Array.isArray(currentProduct.colores_disponibles)
    ) {
      coloresHTML = `
    ...
    if (
      ...
      currentProduct.tipo_prenda_nombre ||
      currentProduct.tipo_tela_nombre
    ) {
      infoAdicional = `
    ...
    if (currentUser) {
      ...
      document
        ...
        .addEventListener("click", handleQuantityChange);
  ...
  async function loadRelatedProducts(categoria, currentId) {
    ...
    if (data && data.length > 0) {
      relatedProductsGrid.innerHTML = data
        .map(
          (product) => `
    ...
    document
      .querySelectorAll(".related-products .product-card")
      .forEach((card) => {
  ...
  function handleAddToCart() {
    ...
    if (quantity > 0) {
      ...
      if (selectedColor) {
        ...
        const colorName =
          selectedColor.querySelector(".color-name").textContent;
  ...
  function renderCartItems() {
    if (cart.length === 0) {
      cartItemsContainer.innerHTML =
        '<p class="empty-cart-message">Tu lista de pedido está vacía.</p>';
      return;
    }
  ...
  function setupEventListeners() {
    ...
    document
      .querySelector(".cart-modal__footer")
