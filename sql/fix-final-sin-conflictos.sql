-- SOLUCIÓN FINAL - ELIMINAR FUNCIONES EXISTENTES PRIMERO
-- Este script elimina TODAS las funciones conflictivas antes de recrear

-- PASO 1: Eliminar TODAS las funciones existentes que pueden causar conflictos
DROP FUNCTION IF EXISTS get_categorias_activas() CASCADE;
DROP FUNCTION IF EXISTS get_tipos_prenda_por_categoria(integer) CASCADE;
DROP FUNCTION IF EXISTS get_tipos_prenda_por_categoria(INTEGER) CASCADE;
DROP FUNCTION IF EXISTS get_tipos_prenda_por_categoria(INTEGER DEFAULT NULL) CASCADE;
...
-- PASO 2: Eliminar vistas que dependen de las columnas
DROP VIEW IF EXISTS vista_productos_completa CASCADE;

-- PASO 3: Eliminar foreign keys existentes
...
-- PASO 4: Eliminar las columnas problemáticas con CASCADE
...
-- PASO 9: Crear funciones RPC NUEVAS (sin conflictos de nombres)
...
CREATE FUNCTION get_tipos_prenda_por_categoria(input_categoria_id INTEGER DEFAULT NULL)
...
BEGIN
    ...
    AND (input_categoria_id IS NULL OR tp.categoria_id = input_categoria_id)
    ORDER BY tp.nombre;
END;
$$ LANGUAGE plpgsql;
...
-- PASO 11: Función simple para dashboard
